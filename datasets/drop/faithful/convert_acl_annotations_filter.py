...
QTYPE2LISP = {
    ...
    "count_filterfind_qtype": "(aggregate_count (filter_passage select_passage))",
    ...
    "relocate_maxfilterfind_qtype": "(select_passagespan_answer (project_passage (select_max_num (filter_passage select_passage))))",
    "relocate_minfilterfind_qtype": "(select_passagespan_answer (project_passage (select_min_num (filter_passage select_passage))))",
    ...
    "num_filterfind_qtype": "(select_num (filter_passage select_passage))",
    "min_find_qtype": "(select_num (select_min_num select_passage))",
    "max_find_qtype": "(select_num (select_max_num select_passage))",
    "max_filterfind_qtype": "(select_num (select_max_num (filter_passage select_passage)))",
    "min_filterfind_qtype": "(select_num (select_min_num (filter_passage select_passage)))",
}
...
def convert_annotations(faithful_dataset, drop_dataset):
    ...
    and "after-max-event"
    """
    print("\nConverting ACL annotations to new lisp/nested-expr/Node stype DROP data processing")
    print("we hope to find all annotated qas in the provided drop subset; if one is not found it will be reported.")
    ...
    for passage_id, passage_info in faithful_dataset.items():
        ...
        for qa in passage_info[constants.qa_pairs]:
            ...
            if qtype == "yeardiff_find_qtype":
                ...
                program_node.extras[FAITHFUL_ANNOTATION_KEY] = {"filter": filter_spans,
                                                                "select": select_spans}
