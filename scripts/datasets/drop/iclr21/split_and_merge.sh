...
ICLR20_DIR=${ROOT_DIR}/iclr20_full-v1
QDMR_DIR=${ROOT_DIR}/qdmr-v1

MERGED_DATA=${ROOT_DIR}/iclr_qdmr-v1

MERGED_DATA_NOEXC=${ROOT_DIR}/iclr_qdmr-v1-noexc

# Splitting ICLR20 into 90 / 10
python -m datasets.drop.split_dev \
  --input_dir ${ICLR20_DIR} \
  --output_dir ${ICLR20_DIR}/iid_split \
  --dev_perc 0.1

# Splitting QDMR into 85 / 15 -- since QDMR is 4762/773(15%) train/dev
python -m datasets.drop.split_dev \
  --input_dir ${QDMR_DIR} \
  --output_dir ${QDMR_DIR}/iid_split \
  --dev_perc 0.15


python -m datasets.drop.merge_datasets \
  --dir1 ${ICLR20_DIR}/iid_split \
  --dir2 ${QDMR_DIR}/iid_split \
